...
describe("Login test", () => {
    ...
    test("닉네임 변경", async () => {
        const response = await request(app).put("/api/user/changenick").set("authorization", `Bearer ${token}`).send({ nickname: testdata.ChangeNick });

        expect(response.statusCode).toBe(200);
    });
    test("이미 존재하는 닉네임인 경우 400", async () => {
        const response = await request(app).put("/api/user/changenick").set("authorization", `Bearer ${token}`).send({ nickname: testdata.nick });

        expect(response.statusCode).toBe(400);
    });
    test("비밀번호 변경", async () => {
        const response = await request(app).put("/api/user/changepassword").set("authorization", `Bearer ${token}`).send({ password: testdata.Pw, newpassword: testdata.ChangePw });
