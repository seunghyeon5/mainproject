...
const recommendDrink = async (req: Request, res: Response) => {
    try {
        ...
        await Drinks.updateMany({ _id: { $in: drinkId } }, { $set: { recommend: cnt + 1 }, $push: { recommend_list: user_id } }).exec();
...
const undoRecommend = async (req: Request, res: Response) => {
    try {
        ...
        await Drinks.updateMany({ _id: { $in: drinkId } }, { $set: { recommend: cnt - 1 }, $pull: { recommend_list: user_id } }).exec();
