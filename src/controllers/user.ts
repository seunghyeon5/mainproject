...
const signup = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (existEmail) {
            return res.status(HttpStatusCode.BAD_REQUEST).json({ result: false, message: "중복된 이메일" });
        }
        // 닉네임 중복확인 버튼
        const existnicName = await User.findOne({ nickname });
        if (existnicName) {
            return res.status(HttpStatusCode.BAD_REQUEST).json({ result: false, message: "중복된 닉네임" });
        }
...
const login = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!user?.email) {
            return res.status(HttpStatusCode.NOT_FOUND).json({ result: false, message: "이메일 없음" });
        }
        const check = await bcrypt.compare(password, user!.password);
        if (!check) {
            return res.status(HttpStatusCode.NOT_FOUND).json({ result: false, message: "비밀번호 불일치" });
        }
        //토큰 발급
...
const checkuser = async (req: Request, res: Response) => {
    ...
    if (!user) {
        return res.status(HttpStatusCode.NOT_FOUND).json({ result: false, message: "존재하지 않음" });
    }
    return res.json({
        result: true,
        message: "success",
...
const changeNickname = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (existNickname) {
            return res.status(HttpStatusCode.BAD_REQUEST).json({ result: false, message: "이미 존재" });
        } else {
...
const changePassword = async (req: Request, res: Response) => {
    ...
    try {
        if (!user) {
            return res.status(HttpStatusCode.NOT_FOUND).json({ result: false, message: "존재하지 않는 유저" });
        }
