...
const user_validation = {
    user_signUp: async (req: Request, res: Response, next: NextFunction) => {
        ...
        const schema = Joi.object().keys({
            nickName: Joi.string().pattern(new RegExp("^[ㄱ-ㅎa-zA-Z0-9가-힣]{1,8}$")).required(), //특수문자만안되고 글자수는 1~8글자
            userAge: Joi.string().min(1).max(3).required(), //숫자만 되고 글자3수
            userGender: Joi.string().required(),
            userInterest: Joi.array().required(),
            address: Joi.string().required(),
            userContent: Joi.string().pattern(new RegExp("^[!?~.^ㄱ-ㅎ|가-힣|a-z|A-Z|0-9 ]{1,100}$")).required()
        });
